...
func TestSettingsPageClearsErrorQueryWhenSuccessExists(t *testing.T) {
	app, database := newOnboardingTestApp(t)
	...
	testCases := []struct {
	...
	}{
		{
			...
			path:         "/settings?error=weak+password&success=cycle_updated",
			location:     "/settings?success=cycle_updated",
			successLabel: "Cycle settings updated successfully.",
		},
		{
			name:         "password status success",
			path:         "/settings?error=weak+password&status=password_changed",
			location:     "/settings?status=password_changed",
	...
	for _, testCase := range testCases {
		t.Run(testCase.name, func(t *testing.T) {
			...
			if location := response.Header.Get("Location"); location != testCase.location {
				t.Fatalf("expected redirect %q, got %q", testCase.location, location)
			}

			followRequest := httptest.NewRequest(http.MethodGet, testCase.location, nil)
			followRequest.Header.Set("Accept-Language", "en")
			followRequest.Header.Set("Cookie", authCookie)
